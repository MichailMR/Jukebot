...
class mediator:
    def __init__(self, channel_id):
        ...
        self.id = channel_id
        self.radio_name, self.play_url, self.voice_id = None, None, None
        
    async def request(self, play_url, radio_name, ctx, voice_client):
            #actually don't want ctx in here, to much connection te class 1
        await self.check_jukebox(ctx, voice_client)
        
        self.radio_name, self.play_url = radio_name, play_url   
        ...
        await voice_client.play(play_url)
    ...
    async def tuner(self, ctx):
        ...
        await self.request(radio_url, radio_name, ctx, voice_client)
        
    async def pause_resume(self, voice_client):
        await voice_client.pause_resume()

    async def check_jukebox(self, ctx, voice_client):
            #Source of the ctx problem (remove ctx)
        ###Create jukebox and retrieve id index
        if (True if voice_client == None else not voice_client.is_connected()):
            voice_channel = ctx.author.voice.channel
            voice_client = jukebox(voice_channel)
            await voice_channel.connect()
            self.voice_id = voice_channel.id
...
class client:
    def __init__(self):
        ...
        async def on_raw_reaction_add(payload):
            ...
            if channel_id == self.pending_options["channel"] and not member_id == self.bot.user.id:
                ...
                if self.pending_options["type"] == 'search':
                    ...
                    await mediator.request(input["play_url"], input["radio_name"], ctx, voice_client) 
                    await ctx.invoke(self.bot.get_command('radio'))
        ...
        async def radio(ctx):
            ...
            if len(likes) > 0 and (True if voice_client == None else not voice_client.is_playing()):
                liked_radio = random.choice(likes)
                
                await mediator.request(liked_radio["play_url"], liked_radio["radio_name"], ctx, voice_client) 
        ...
        async def streamurl(ctx, *args):
            ...
            if len(args) > 0:
                ...
                await mediator.request(input, 'user input', ctx, voice_client)
